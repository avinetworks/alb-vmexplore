...
resource "null_resource" "update_ip_to_client_dhcp" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo netplan apply",
      "sudo useradd -m ${var.ubuntuuser}",
      "echo \"${var.ubuntuuser}:${var.ubuntuuserpass}\" | sudo chpasswd"
    ]
  }
}
...
resource "null_resource" "traffic_gen" {
  provisioner "local-exec" {
    command = "echo 'for i in {1..300}; do curl -k https://ingress.cluster1.${var.avi_domain}; sleep 0.8 ; done' | tee traffic_gen.sh"
  }
}

resource "null_resource" "traffic_gen_copy_static" {
...
resource "null_resource" "traffic_gen_copy_dhcp" {
  depends_on = [null_resource.traffic_gen]
  count = (var.vcenter_network_mgmt_dhcp == true ? 1 : 0)
  connection {
    host        = vsphere_virtual_machine.client_dhcp[0].default_ip_address
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "(crontab -l 2>/dev/null; echo \"*/1 * * * * /bin/bash /home/${var.client.username}/traffic_gen.sh\") | crontab -"
    ]
  }
