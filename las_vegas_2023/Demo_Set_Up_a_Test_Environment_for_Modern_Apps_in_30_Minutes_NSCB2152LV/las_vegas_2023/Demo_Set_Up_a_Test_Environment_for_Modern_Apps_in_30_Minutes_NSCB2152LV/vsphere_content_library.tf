...
resource "null_resource" "download_ubuntu_focal" {
  provisioner "local-exec" {
    command = "wget -q -O /tmp/$(basename ${var.content_library.source_url_ubuntu_focal}) ${var.content_library.source_url_ubuntu_focal}"
  }
}
...
resource "vsphere_content_library" "library_se" {
  name            = "${var.content_library.basename}-se-${random_string.id.result}"
  storage_backing = [data.vsphere_datastore.datastore.id]
}
