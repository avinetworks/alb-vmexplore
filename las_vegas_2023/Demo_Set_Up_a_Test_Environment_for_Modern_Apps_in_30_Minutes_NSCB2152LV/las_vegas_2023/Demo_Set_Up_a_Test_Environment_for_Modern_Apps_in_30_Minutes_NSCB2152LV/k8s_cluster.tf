...
resource "null_resource" "ako_deploy" {
  ...
  provisioner "remote-exec" {
    inline = [
      "helm --debug install ako/ako --generate-name --version ${var.ako_version} -f values.yml --namespace=avi-system  --set loglevel=${var.loglevel} set avicredentials.username=admin --set avicredentials.password=${random_string.password.result}"
    ]
  }
}
